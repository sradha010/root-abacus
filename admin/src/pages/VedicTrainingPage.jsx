...
const VedicTrainingPage = () => {
  ...
  const fetchAll = async () => {
    try {
      const { data } = await API.get('/teacher-training?type=vedic')
      setItems(data.data)
    } catch {}
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(form).forEach(([key, val]) => {
        if (key === 'highlights') {
          val.forEach(h => formData.append('highlights[]', h))
        } else {
      ...
      if (editId) {
        await API.put(`/teacher-training/${editId}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } })
        setMsg('Updated successfully!')
      } else {
        await API.post('/teacher-training', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
  ...
  const handleEdit = (item) => {
    setForm({ ...item, highlights: item.highlights || [] })
    setEditId(item._id)
    setShowForm(true)
    window.scrollTo(0, 0)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-gray-800">Vedic Maths Teacher Training</h2>
        {!showForm && (
          <button
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input name="slug" value={form.slug} onChange={handleChange} required
                placeholder="vedic-maths-teacher-training"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
            </div>
            ...
            <div>
              ...
              <input name="duration" value={form.duration} onChange={handleChange}
                placeholder="e.g. 20 days self-paced"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
            </div>
            ...
            <div>
              ...
              <input name="fee" value={form.fee} onChange={handleChange}
                placeholder="e.g. INR 7900 / USD 120"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex flex-wrap gap-2">
                {form.highlights.map((h, i) => (
                  <span key={i} className="flex items-center gap-1 bg-orange-50 text-orange-700 text-xs px-3 py-1 rounded-full">
                    {h}
                    <button type="button" onClick={() => handleRemoveHighlight(i)} className="ml-1 text-orange-400 hover:text-red-500">×</button>
                  </span>
                ))}
              </div>
            </div>
            ...
            {form.videoType === 'upload' && (
              <div className="md:col-span-2">
                ...
                {form.videoFile && !videoFile && (
                  <p className="text-xs text-gray-500 mt-1">Current: <a href={form.videoFile} target="_blank" rel="noreferrer" className="text-orange-500 underline">View video</a></p>
                )}
              </div>
            )}
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {items.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8 text-gray-400">No Vedic Maths training programs yet.</td></tr>
            ) : items.map((item) => (
              <tr key={item._id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-600">{item.fee || '—'}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-1 rounded text-xs ${item.videoType !== 'none' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
                    {item.videoType === 'youtube' ? 'YouTube' : item.videoType === 'upload' ? 'Uploaded' : 'None'}
                  </span>
                </td>
                <td className="px-4 py-3">
...
export default VedicTrainingPage