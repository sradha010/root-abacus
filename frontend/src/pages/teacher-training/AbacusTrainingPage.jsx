...
import {
  Monitor, GraduationCap, Globe, UserCheck, Briefcase, Users,
  Home, BookOpen, Building, School, ShieldCheck, Heart,
  IndianRupee, Clock, Zap, Video, FileText, Smartphone,
  Award, Timer, Calendar, Brain, Rocket, BarChart2,
  Headphones, MapPin, CheckCircle, Info, Lock,
} from 'lucide-react'
...
const highlights = [
  {
    ...
    custom: (
      <div>
        <p className="line-through text-gray-400 text-sm">INR 45000 (56% Off)</p>
        <p className="text-[#E87722] font-bold">Effective Price: INR 19900 / USD 275</p>
        <p className="text-gray-400 text-xs italic">Inclusive of taxes – Limited Period Offer</p>
      </div>
    ),
  },
  { feature: 'Duration',             detail: 'Self-paced. Finish in as little as 40 days' },
  { feature: 'Levels Covered',       detail: '8 Levels' },
  { feature: 'Method of Training',   detail: 'Easy-to-follow online abacus training video modules' },
...
const whoShouldEnroll = [
  { icon: Home,     title: 'Educated Homemakers',      desc: 'Turn spare time into income' },
  { icon: BookOpen, title: 'Tutors & Coaching Institutes', desc: 'Add a trending course' },
  { icon: School,   title: 'School Teachers',          desc: 'Start your weekend center' },
  { icon: Briefcase, title: 'Career Seekers',          desc: 'Build a flexible and fulfilling profession' },
...
const howItWorks = [
  { n: 1, title: 'Video-Based Learning',      desc: 'Simple videos for every concept' },
  { n: 2, title: 'Practice Worksheets',       desc: 'Auto-evaluated, repeatable' },
  { n: 3, title: 'Flexible Timing',           desc: 'Just 30 mins/day to complete in 40 days' },
  { n: 4, title: 'Topics Covered',            desc: 'Addition, Subtraction, Multiplication, Division, on Abacus and Mentally' },
  { n: 5, title: '100% Online | Mobile Friendly', desc: 'No Tech Skills Required' },
]

const abacusCourse = [
  { icon: Timer,     title: 'Duration',       desc: '2 Years | Levels: 8 (3 months each)' },
  { icon: Calendar,  title: 'Class Timing',   desc: 'Weekly 2 hours' },
  { icon: Brain,     title: 'Daily Practice', desc: '15 mins (manual or online)' },
  { icon: Rocket,    title: 'Objective',      desc: 'Fast, accurate mental maths without pen & paper' },
  { icon: UserCheck, title: 'Ideal Age',      desc: '5–12 years' },
  { icon: Award,     title: 'Benefits',       desc: 'Focus, memory, observation & calculation power' },
]
...
const AbacusTrainingPage = () => {
  ...
  const [program, setProgram]           = useState(null)   // teacher-training record from DB
  const [videoLoading, setVideoLoading] = useState(true)

  useEffect(() => {
    // Fetch the active abacus teacher-training program for its video
    fetch(`${API_BASE}/teacher-training?type=abacus`)
      .then(r => r.json())
      .then(d => {
        if (d.success && d.data?.length > 0) {
          // Pick the first active record (admin controls order via `order` field)
          setProgram(d.data[0])
        }
      })
      .catch(() => {})
  ...
  const hasVideo =
    program &&
    program.videoType !== 'none' &&
    (program.videoUrl || program.videoFile)

  return (
    <>
      {/* ── HERO ── */}
      <section className="w-full py-10 px-4 text-center" style={{ backgroundColor: '#fef4ea' }}>
        <div className="max-w-4xl mx-auto">
          ...
          {/* ── DYNAMIC VIDEO ── */}
          {videoLoading ? (
            <div className="w-full max-w-3xl mx-auto pb-[56.25%] relative rounded-2xl overflow-hidden bg-gray-200 animate-pulse" />
          ) : hasVideo ? (
      ...
      <section className="py-12 px-6 bg-gradient-to-r from-orange-50 to-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            With growing awareness of the lifelong benefits of Abacus education, more parents, schools, and tutors are seeking abacus training and certified Abacus trainers. Abacus is not just a maths tool—it's a brain development program that builds focus, memory, and mental calculation skills.
          </p>
          <p className="text-gray-600 mb-4 leading-relaxed">
            Abacus trainers enjoy flexible working hours, impressive earnings, and the joy of shaping young minds—all from the comfort of their home.
          </p>
          <p className="text-[#E87722] text-sm italic font-medium">
            <strong>Fact:</strong> Children start showing improved mental maths within just 4 weeks of learning Abacus!
          </p>
        </div>
      </section>
      ...
      <section className="py-12 px-6 bg-white">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
            {highlights.map((row, i) => (
              <div key={i} className={`grid grid-cols-2 px-6 py-4 border-b border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                ...
                {row.custom ? row.custom : <span className="text-gray-600">{row.detail}</span>}
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 bg-orange-50 rounded-2xl p-6">
            <h3 className="text-lg font-bold text-[#E87722] mb-4">Other Benefits After Training Completion</h3>
      ...
      <section className="py-12 px-6 bg-orange-50">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold text-center text-[#E87722] mb-8">Who Should Enroll?</h2>
          ...
          <div className="bg-white rounded-2xl p-6 border border-orange-100">
            ...
            <p className="text-gray-700 text-sm mb-2">
              Passion for teaching, basic education (preferably graduate), and approx. 60 hours of learning commitment.
            </p>
      ...
      <section className="relative py-12 px-6 bg-white overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <h2 className="text-2xl md:text-3xl font-bold text-center text-[#E87722] mb-10">The Complete Advantage with Roots16</h2>
          <div className="grid md:grid-cols-3 gap-6">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="w-12 h-12 rounded-full bg-orange-100 flex items-center justify-center mb-4">
                <Monitor size={24} strokeWidth={1.5} color="#E87722" />
              </div>
              <h3 className="font-bold text-[#E87722] mb-4">Where Can You Teach?</h3>
              <ul className="space-y-2">
                {whereTeach.map((item, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-gray-600">
                    <CheckCircle size={15} strokeWidth={1.5} color="#E87722" className="mt-0.5 flex-shrink-0" />
                    {item}
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <h3 className="font-bold text-[#E87722] mb-4">Why Trainers Love Abacus Teaching?</h3>
              <ul className="space-y-2">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <h3 className="font-bold text-[#E87722] mb-4">Why Choose Roots?</h3>
              <ul className="space-y-2">
      ...
      <section className="py-12 px-6 bg-orange-50">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold text-center text-[#E87722] mb-8">Why Online Learning Works Best?</h2>
      ...
      <section className="py-12 px-6 bg-white">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold text-center text-gray-800 mb-10">How Training Works</h2>
      ...
      <section className="py-12 px-6 bg-orange-50">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl p-8 flex gap-6 items-start shadow-sm">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-800 mb-2">ISO-Certified Certification</h3>
              <p className="text-gray-600 text-sm mb-4">
                Upon completion, receive an official Roots Abacus Learning School Certificate. Our online exams are designed to evaluate real understanding and skills.
              </p>
              <ul className="space-y-2">
                {['Online exam format', 'Receive certificate instantly', 'Globally recognized', 'Share on LinkedIn & resume'].map((item, i) => (
      ...
      <section className="py-12 px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold text-center text-[#E87722] mb-8">Abacus Course for Kids</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <section className="py-12 px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold text-center text-gray-800 mb-10">What Our Trainers Say</h2>
      ...
      <section className="py-16 px-6 text-center bg-orange-50">
        <h2 className="text-3xl font-bold text-[#E87722] mb-3">Ready to Start Your New Career?</h2>
        <p className="text-gray-600 mb-2">Turn your passion into a profession. Inspire children. Grow your income.</p>
        <p className="text-[#E87722] font-semibold mb-8">Become a Certified Abacus Trainer with Roots Today!</p>
